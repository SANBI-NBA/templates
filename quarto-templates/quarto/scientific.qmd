...
<!--# Key statistics section: This section can be edited to report more/fewer statistics by adding more items to each column (these will be stacked), or adding more columns (then they will be placed next to each other). Using source view, copy and paste the entire block (the three lines with the pipe | at the start), and edit contents as necessary. Do not change any of the inline styling {the part in the squiggly brackets}, [other than the colour applied to the statistic](@Lize, it isn't clear to me why you mention changing colour here, as that is specified in the custom.scss and surely we don't want users to go digging in there?). Remember to adjust the column widths to add up to 100%. Try not to add more columns, as more than two columns tend to display poorly on narrow screens. Delete the entire section if you don't want to include statistics on your page-->
